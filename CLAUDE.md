...
A football (soccer) prediction platform. A **Tournament** is the shared real-world competition (its phases/groups/fixtures and official results); a **Pool** is a playable contest (a "bolão") over a tournament, owning its own scoring strategy/config, prediction lock, and `source` (the group that created it, e.g. "FF&A"). Many pools can run over one tournament, differing only by scoring. Players submit one **Entry** per **Pool** and predict fixture outcomes; group-stage score predictions are resolved into a full knockout bracket. Every pool is currently open to every user (a "join a pool" feature is future work). Stack: Laravel 13 + Inertia v3 + React 19 (SPA, no client router), MySQL (local) / SQLite (CI), Tailwind v4, Wayfinder. Platform brand "Brothers Bets" (pitch-green + gold); the seeded pool is "World Cup 2026" (source "FF&A", emphasised in the UI) over the "World Cup 2026" tournament.

> Scoring lives in `app/Services/Scoring` and reads `Pool.scoring_strategy`/`scoring_config`. Approving an official-result batch is a **tournament** action that cascades to **every pool** of that tournament — re-scoring each pool's entries and snapshotting its leaderboard.
...
- `Tournament` owns the competition structure + lifecycle `status` (and `transitionTo()`/`TournamentStatusChanged`). `Pool` belongs to a `Tournament` and owns `scoring_strategy`/`scoring_config`/`predictions_lock_at`/`source` + its `entries()`. Both carry a `slug`; **pool routes bind `{pool:slug}`** (a `Pool`, reaching structure via `$pool->tournament`) — there is no `getRouteKeyName()` override anywhere. `Pool::acceptsPredictions()` (via `Pool::predictionsLockAt()`) **derives** the group-stage lock from the tournament's first group kickoff minus a buffer (`config('scoring.prediction_lock_buffer_minutes')`, default 60), unless an explicit `predictions_lock_at` override is set (which wins verbatim, ignoring the buffer; null override + no scheduled kickoff = closed). It is **deliberately independent of the tournament's `status`**. Phased-bracket pools lock each knockout round the same buffer before that round's first kickoff (`PredictionWindowResolver`); `Pool::usesPhasedPredictionWindows()`/`predictsKnockoutBracket()` key off `scoring_strategy`. The seeded pools set no override, so they derive. The seeded tournament keeps the slug `world-cup-2026`; its pools use `world-cup-2026-ffa` and `world-cup-2026-brothers`.
...
- Routes: `web.php` (public `home` + the only unauthenticated POST `login.code.send`), `pools.php` (all `auth`-gated, the app core), `settings.php`, `console.php`. Middleware is appended to the `web` group in `bootstrap/app.php`.
...
- Status transitions: PATCH `pools/{slug}/status` → invokable `TransitionTournamentController`, authorized by the `manage-tournament` Gate (admin). `TournamentStatusChanged` is fired but currently has **zero listeners** (future hook).
...
- **Layouts are assigned centrally** in `resources/js/app.tsx` via `createInertiaApp({ layout })` keyed on page name (e.g. `settings/*` → nested `[AppLayout, SettingsLayout]`, `pools/index` → `HubLayout`, rest → `AppLayout`). Pages pass per-page config (breadcrumbs/title) via a **static `.layout` property** on the component, not props. The sidebar switches to tournament-context nav based on a server-shared `pool` prop.
