...
- **NEVER translate** the brand "Brothers Bets" or a pool's `source`. A pool reads as `{source} · {translated tournament name}`; the English possessive ("X's pool") becomes "bolão da {source}". Money is already pt-BR (`lib/money.ts`).
