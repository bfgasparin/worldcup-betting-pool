...
- Other controllers: `PoolController` (`index`/`show`/`join`/`markBriefingSeen`/`leaderboard`), `ManageController` (admin landing), `OnboardingController` (the wizard), and `Settings\ProfileController`/`Settings\SecurityController`.
- `HandleInertiaRequests` shares `auth.user`, `auth.isAdmin`, `timezone`, `sidebarOpen`, and `joinedPools` (the user's pools for the sidebar, each with a `needsAttention` dot, built query-light by `App\Services\Pools\JoinedPools`) on every request; `HandleAppearance` shares the `appearance` cookie to Blade for SSR theming. The `needsAttention`/completion UI is fed by `AttentionSummary`/`CompletionSummary`/`PredictionAttention` (`app/Services/Pools`), and `PlayerComparison` (`app/Services/Predictions`) powers the compare strip/dock.

## Frontend (`resources/js`, `resources/css/app.css`)

- Inertia v3 SPA, **React Compiler enabled** (`babel-plugin-react-compiler` in `vite.config.ts`). Pages in `resources/js/pages/` (`pools/{index,show,predict,leaderboard}`, `live/{index,show}`, `manage/{index,live,schedule,scores}`, `onboarding/wizard`, `settings/{profile,security,appearance}`, `auth/login`, `welcome`); `@/*` aliases `resources/js/*`.
