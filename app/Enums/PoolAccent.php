...
/**
 * The colour identity given to a pool so that pools played over the *same* tournament — which
 * otherwise share a name, dates and sport — read as distinct at a glance, everywhere the pool is
...
enum PoolAccent: string
{
    ...
    /**
     * The accent for a pool at the given 0-based position among its tournament's pools, cycling
     * the cases. Mirrors `poolAccent()` in `resources/js/lib/accents.ts`, and is used to assign a
     * distinct default accent to each pool a factory builds.
     */
     ...
     * (gold and teal especially) never produce an illegible eyebrow. Guarded by PoolAccentTest.
     */
