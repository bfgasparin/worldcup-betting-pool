...
class PhasedPredictionControllerTest extends TestCase
{
    ...
    private function phasedGame(\DateTimeInterface|string|null $lockAt): Game
    {
        return Game::factory()->phasedBracket()->create([
    ...
    public function test_predict_page_shows_official_knockout_teams_and_phase_windows(): void
    {
        ...
        $this->actingAs($this->user)
            ->get(route('games.predict.edit', $game->slug))
            ->assertOk()
            ->assertInertia(fn (AssertableInertia $page) => $page
                ->component('games/predict')
                ->where('game.scoring_strategy', 'phased-bracket')
                ->where('game.can_edit', false)            // the group window is shut
    ...
    public function test_saving_a_knockout_round_persists_the_scoreline_without_cascading(): void
    {
        ...
        $this->actingAs($this->user)
            ->put(route('games.predict.knockout', $game->slug), ['predictions' => [[
                'fixture_id' => $r32->id,
                'home_goals' => 2,
                'away_goals' => 1,
            ]]])
            ->assertRedirect(route('games.predict.edit', $game->slug));

        $entry = Entry::where('game_id', $game->id)->where('user_id', $this->user->id)->firstOrFail();
    ...
    public function test_group_save_does_not_create_knockout_rows(): void
    {
        ...
        $this->actingAs($this->user)
            ->put(route('games.predict.group', $game->slug), $payload)
            ->assertRedirect(route('games.predict.edit', $game->slug));

        $entry = Entry::where('game_id', $game->id)->where('user_id', $this->user->id)->firstOrFail();
        $this->assertSame(6, $entry->groupPredictions()->count());
        // Phased games never cascade group scores into a self-derived bracket.
