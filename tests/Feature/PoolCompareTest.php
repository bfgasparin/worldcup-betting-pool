...
/**
 * The "compare players" payload on the pool detail page. The load-bearing concern is the anti-cheat
 * gate: another player's predictions are revealed only once their prediction window has locked. Lock
 * state is set explicitly via {@see Pool::$predictions_lock_at} (past = locked, future = open) so the
 * tests never depend on the wall clock relative to the seeded 2026 kickoffs.
 */
class PoolCompareTest extends TestCase
{
    use RefreshDatabase;
    ...
    {
        $this->pool->update(['predictions_lock_at' => now()->addDay()]);
    }

    private function entryFor(string $name): Entry
    {
        return Entry::factory()->for($this->pool)->for(User::factory()->create(['name' => $name]))->create();
    }
    ...
    public function test_no_comparison_in_normal_mode(): void
    {
        ...
        Entry::factory()->for($this->pool)->for($user)->create();
    ...
    public function test_comparison_players_expose_avatar_urls(): void
    {
        ...
        $this->actingAs($viewer)
            ->get(route('pools.show', ['pool' => 'world-cup-2026-ffa', 'compare' => (string) $opponent->id]))
            ->assertInertia(fn (AssertableInertia $page) => $page
    ...
    public function test_an_empty_or_garbage_compare_param_yields_no_comparison(): void
    {
        ...
        foreach (['', 'abc', '0', ' , '] as $value) {
            $this->get(route('pools.show', ['pool' => 'world-cup-2026-ffa', 'compare' => $value]))
                ->assertOk()
    ...
    public function test_comparison_lists_the_viewer_first_then_opponents_in_order(): void
    {
        ...
        $this->actingAs($user)
            ->get(route('pools.show', ['pool' => 'world-cup-2026-ffa', 'compare' => "{$ana->id},{$bruno->id}"]))
            ->assertOk()
            ->assertInertia(fn (AssertableInertia $page) => $page
    ...
    public function test_an_opponents_knockout_pick_is_gated_by_its_window(): void
    {
        ...
        $this->keepOpen();
        $this->get(route('pools.show', ['pool' => 'world-cup-2026-ffa', 'compare' => (string) $opponent->id]))
            ->assertInertia(fn (AssertableInertia $page) => $page
    ...
    public function test_the_viewers_own_picks_are_always_visible_even_before_lock(): void
    {
        ...
        $user = User::factory()->create();
        $entry = Entry::factory()->for($this->pool)->for($user)->create();
        $opponent = $this->entryFor('Rival');
        $fixture = $this->groupA()->fixtures()->orderBy('match_number')->first();
    ...
    public function test_the_comparison_is_capped_at_three_opponents(): void
    {
        ...
        $this->actingAs($user)
            ->get(route('pools.show', ['pool' => 'world-cup-2026-ffa', 'compare' => $ids]))
            ->assertInertia(fn (AssertableInertia $page) => $page
    ...
    {
        $user = User::factory()->create();
        $mine = Entry::factory()->for($this->pool)->for($user)->create();
        $opponent = $this->entryFor('Rival');

        $this->actingAs($user)
            ->get(route('pools.show', ['pool' => 'world-cup-2026-ffa', 'compare' => "{$mine->id},{$opponent->id}"]))
            ->assertInertia(fn (AssertableInertia $page) => $page
    ...
    public function test_foreign_and_nonexistent_entry_ids_are_dropped(): void
    {
        ...
        // An entry that belongs to the sibling pool, plus an id that doesn't exist at all.
        $brothers = $this->tournament->pools()->where('slug', 'world-cup-2026-brothers')->firstOrFail();
        $foreign = Entry::factory()->for($brothers)->create();

        $this->actingAs($user)
            ->get(route('pools.show', ['pool' => 'world-cup-2026-ffa', 'compare' => "{$opponent->id},{$foreign->id},99999999"]))
            ->assertInertia(fn (AssertableInertia $page) => $page
                ->has('comparison.players', 2)
                ->where('comparison.players.1.entry_id', $opponent->id)
            );
    ...
    {
        $user = User::factory()->create(['name' => 'Me']);
        Entry::factory()->for($this->pool)->for($user)->create(['total_points' => 10]);
        Entry::factory()->for($this->pool)->for(User::factory()->create(['name' => 'Top']))->create(['total_points' => 99]);
