...
class GameAccentTest extends TestCase
{
    /**
    ...
    {
        foreach (GameAccent::cases() as $accent) {
            $ratio = $this->contrastWithWhite($accent->eyebrowInk());

            $this->assertGreaterThanOrEqual(
                4.5,
                $ratio,
                "GameAccent::{$accent->name} eyebrow ink {$accent->eyebrowInk()} only reaches "
    ...
    public function test_gradient_hex_matches_the_css_custom_properties(): void
    {
        ...
        $vars = [
            'pitch' => GameAccent::Pitch,
            'teal' => GameAccent::Teal,
            'gold' => GameAccent::Gold,
            'violet' => GameAccent::Violet,
        ];
        ...
        foreach ($vars as $name => $accent) {
            ...
            $this->assertSame(strtolower($accent->gradientFrom()), strtolower($matches[1]), "GameAccent::{$accent->name} 'from' hex drifted from --g-{$name}.");
            $this->assertSame(strtolower($accent->gradientTo()), strtolower($matches[2]), "GameAccent::{$accent->name} 'to' hex drifted from --g-{$name}.");
        }
    }
